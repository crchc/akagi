...
| `majsoul_rematch.rs` | Watches confirmed Mahjong Soul game ends, recognizes the result buttons in a clipped canvas screenshot, and sends the two result confirmations, rematch press, and matchmaking confirmation through CDP. `autoplay.majsoul.total_games` includes the current game; 1 disables rematching. The count lasts for this process run. |
