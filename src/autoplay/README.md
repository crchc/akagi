...
| `majsoul_rematch.rs` | Watches confirmed Mahjong Soul game ends. Full viewport screenshots use the live canvas rect and rounded 1600×900 button regions; unknown screens advance at the Rematch position for up to 120 seconds. A visible match dialog confirmation takes priority even if Rematch was clicked during an unknown screen. `autoplay.majsoul.remaining_games` includes the current game, decreases after each confirmed game, and can be edited during play. Zero stops rematching. |
