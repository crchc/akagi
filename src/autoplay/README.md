...
| `majsoul_rematch.rs` | Watches confirmed Mahjong Soul game ends. Full viewport screenshots recognize rematch and matchmaking buttons; bounded confirmation clicks advance result and reward screens. `autoplay.majsoul.remaining_games` includes the current game, decreases after each confirmed game, and can be edited during play. Zero stops rematching. |
